...
def main_gui_wrapper(credentials_path, token_path, spreadsheet_id_input, template_pptx, output_pptx, font_settings, log_func=None):
    ...
    option_columns = [header for header in headers if re.match(r'^Option\s+\S+', header)]
        ...
        for shape in slide.shapes:
            if shape.has_text_frame:
                ...
                if '[question]' in text:
                    ...
                    # Extract the option key from the placeholder, e.g., [option_ক]
                    option_key_match = re.match(r'\[option_(.+)\]', text)
                    if option_key_match:
                        option_key = option_key_match.group(1)
                        option_column = f"Option {option_key}"
                        option_text = row_data.get(option_column, '')
