...
def parse_docx(docx_path):
    """
    Parses the Word document to extract MCQs (both single- and multi-line options).
    Detects question numbering in Bangla or English digits,
    and captures options whether they appear each on its own line or multiple in one line.
    """
    ...
    # Keep track of all serial numbers if needed
    serial_numbers_seen = set()

    # ========== Regular Expressions ==========
    # 1) Detect MCQ question number: starts with either Bengali (০-৯) or English (0-9), followed by ., ), বা ।
    mcq_question_pattern = re.compile(r'^([\u09E6-\u09EF0-9]+)[.,):।]\s*(.*)', re.UNICODE)

    # 2) Detect main options (single-line): starts with ক), খ), গ), ঘ) or ক., খ., etc.
    option_pattern_main = re.compile(r'^([ক-ঘ])[).]\s*(.*)', re.UNICODE)

    # 3) Detect multiple options in the same paragraph (e.g. "ক. text খ. text গ. text ঘ. text")
    #    This will find all occurrences like:
    #    ক. [some text] খ. [some text] গ. [some text] ঘ. [some text]
    #    or
    #    ক) [some text] খ) [some text] ... etc.
    multiple_option_pattern = re.compile(r'([ক-ঘ])[).]\s*([^ক-ঘ]+)', re.UNICODE)

    # 4) Detect answer line
    answer_pattern = re.compile(r'^উত্তর[:ঃ]\s*(.*)', re.UNICODE)

    # 5) Board/Institute in [brackets]
    ...
    for idx, para in enumerate(document.paragraphs):
        ...
        # If we expected a board/institute from the next paragraph
        if expect_board_institute:
            ...
            if board_institute_match:
                ...
                expect_board_institute = False
                # We'll continue processing this as normal text

        # Check if the paragraph starts a new MCQ question
        mcq_question_match = mcq_question_pattern.match(text)
        if mcq_question_match:
            # If there was an ongoing MCQ, save it
            if serial_number:
                mcq_data.append([
                    serial_number,
                    question_text.strip(),
            ...
            # Start new MCQ
            serial_number = mcq_question_match.group(1)  # the numbering
            ...
            expect_board_institute = True  # Next paragraph might be [board/institute]
            continue

        # Check if this paragraph is an answer line
        ...
        if answer_match:
            ...
            # If it's a single letter like 'ক' or 'খ', map to that option text
            single_option_match = re.match(r'^([ক-ঘ])(\.|$|\s+)', answer_text)
            if single_option_match:
                # The user might have put 'খ.', or just 'খ' or 'খ ')
                # Extract the letter from group(1)
                mapped_letter = single_option_match.group(1)
                if mapped_letter in options and options[mapped_letter]:
                    answer = options[mapped_letter].strip()
                    print(f"Mapped Answer '{mapped_letter}' to '{answer}'")
                else:
                    # if no mapped letter in options, fallback to the entire text
                    answer = answer_text
                    print(f"Assigned entire Answer: {answer}")
            else:
                # If user typed the entire option like "খ. হায়াৎ মামুদ"
                # or a custom text, just store it as-is
                answer = answer_text
                print(f"Assigned Answer: {answer}")
            continue

        # Check for single main option if the entire paragraph has one
        ...
        if option_match_main:
            ...
            current_option = key
            print(f"Detected MCQ {serial_number} - Single Option {key}: {opt_text}")
            continue

        # Check for multiple options in the same paragraph
        option_line_matches = multiple_option_pattern.findall(text)
        if option_line_matches:
            # This means we have multiple options in one line
            for (letter, content) in option_line_matches:
                letter = letter.strip()        # e.g. 'ক', 'খ', 'গ', 'ঘ'
                opt_text = replace_latex_delimiters(content.strip())
                options[letter] = opt_text
                current_option = letter
                print(f"Detected MCQ {serial_number} - Option {letter}: {opt_text}")
            continue

        # If no recognized pattern, treat it as either question text or continuation of the last option
        if serial_number:
            if current_option:
                # Append to the current option if it was in progress
                options[current_option] += ' ' + replace_latex_delimiters(text)
                print(f"Appending to MCQ {serial_number} - Option {current_option}: {text}")
            else:
                # Otherwise, assume it's more text for the question
                question_text += ' ' + replace_latex_delimiters(text)
                print(f"Appending to MCQ {serial_number} - Question: {text}")

    # After finishing all paragraphs, append the last MCQ if it exists
    if serial_number:
        mcq_data.append([
            serial_number,
            question_text.strip(),
...
def write_to_google_sheets(mcq_data, creds, sheet_name):
    ...
    if mcq_data:
        ...
        mcq_values = [header_row] + mcq_data
...
class DocxToGsheetApp:
    ...
    def convert(self):
        ...
        messagebox.showinfo(
            "Success",
            f"Successfully converted and uploaded MCQs to Google Sheets named '{sheet_name_input}'!"
        )
